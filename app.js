// app.js — 完全版（自動再生機能統合済み）
// === ここからコピーペーストすればそのまま動作します ===
...
function renderList(list){
  ...
  list.forEach((w,i)=>{
    ...
    div.innerHTML = `<div><div style="font-weight:700">${w.english}</div><div style="font-size:12px;color:#666">${w.katakana||''} ${w.japanese||''}</div></div><div><button class="btn small" data-index="${i}">学習</button></div>`;
    container.appendChild(div);
  });
  container.querySelectorAll('button[data-index]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const idx = Number(btn.getAttribute('data-index'));
      const gIndex = words.findIndex(x=>x.english===list[idx].english);
      if(gIndex>=0){ currentIndex=gIndex; showAnswer=false; repeatCount=0; renderCard(); el('studyArea').style.display='block'; el('controls').style.display='flex'; }
    });
  });
}
...
function renderCard(){
  ...
  el('counter').textContent = `${currentIndex+1}/${words.length} (${repeatCount+1}/${settings.repeatCount})`;
...
function showHardOnly(){
  ...
  const f = words.filter(w=> list.includes(w.english + '||||' + (w.japanese||'')));
  renderList(f);
}

function addHistory(word){ try{ const h=JSON.parse(localStorage.getItem(LS_HISTORY)||'[]'); h.push({word:word.english,date:new Date().toISOString()}); localStorage.setItem(LS_HISTORY,JSON.stringify(h)); }catch(e){} }

function playAudioForWord(text){
  const fname = sanitizeFilename(text);
  const mp3 = 'audio_mp3/'+fname+'.mp3';
  fetch(mp3,{method:'HEAD'}).then(r=>{ if(r.ok) playAudio(mp3); else if(settings.ttsFallback) speakText(text); }).catch(()=>{ if(settings.ttsFallback) speakText(text); });
}
function playAudio(url){ if(audio) audio.pause(); audio = new Audio(url); audio.play().catch(()=>{}); }
function speakText(t){ if('speechSynthesis' in window){ speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(t); u.lang='en-US'; u.rate=0.95; speechSynthesis.speak(u); }}

// === 自動再生機能 ===
function startAutoPlay(){
  stopAutoPlay();
  const speed = Number(el('autoSpeed')?.value || 2000);
  autoTimer = setInterval(()=>{ cardClicked(); }, speed);
}
function stopAutoPlay(){ if(autoTimer){ clearInterval(autoTimer); autoTimer=null; }}

function addSwipe(elm){
  let sx=0,st=0;
  elm.addEventListener('touchstart',e=>{ const t=e.touches[0]; sx=t.clientX; st=Date.now(); },{passive:true});
  elm.addEventListener('touchend',e=>{ const t=e.changedTouches[0]; const dx=t.clientX-sx; const dt=Date.now()-st; if(Math.abs(dx)>50&&dt<500){ if(dx<0) nextWord(); else if(currentIndex>0){ currentIndex--; renderCard(); }} });
}

window.addEventListener('load',()=>{
  loadSettings(); loadWords();
